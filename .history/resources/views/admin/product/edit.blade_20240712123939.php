...
<form action="{{route('product.update',$category->id)}}" method = 'post' role="form">
    ...
    <div class="mb-3">
      ...
      <input type="text" class="form-control" name='content' value="{{$product->content}}" id="exampleInputEmail1" aria-describedby="emailHelp">
    ...
    <div class="mb-3">
      <label for="exampleInputEmail1" class="form-label">Category_id</label>
       <select id="category_id" name="category_id" class="form-control" required>
            ...
            @foreach ($cats as $cat)
                <option value="{{ $cat->id }}" {{$cat}}>{{ $cat->name }}</option>
            @endforeach
        </select>
